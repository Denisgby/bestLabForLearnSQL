...
SELECT contactname,
	...
	CHARINDEX(N',', contactname) as 'CarIndex befor ","',
	CHARINDEX(N',', contactname)+1 as 'CarIndex befor ","+1',
	LEN(contactname) as dlinna
...
---------------------------------------------------------------------
select contactname, len( REPLACE(contactname, 'a', 'aa'))- LEN(contactname)as qty
from sales.Customers
order by qty desc