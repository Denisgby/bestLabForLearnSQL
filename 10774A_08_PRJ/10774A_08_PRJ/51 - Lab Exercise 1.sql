...
---------------------------------------------------------------------
select N'The unit price for the ' + productname+N' is' + convert ( nvarchar, unitprice)+N' $.' as productdesc
...
--and an end date 11/30/2007 (using US English form, read as November 30, 2007).
--Write a SELECT statement against the Sales.Orders table to retrieve the orderid, orderdate, shippeddate, and shipregion columns. Filter the result to include only rows with the order date between the specified start date and end date and have more than 30 days between the shipped date and order date. Also check the shipregion column for missing values. If there is a missing value, then return the value ‘No region’.
--
...
---------------------------------------------------------------------
select orderid, orderdate, shippeddate, shipregion, coalesce(shipregion, 'not region') as shipregion
from Sales.Orders
where orderdate between '1.04.2007' and '30.11.2007'
...
select  try_CONVERT(  int ,  REPLACE ( replace (replace (phone,'-',''), '(',''), ')', '')) as mun1,
REPLACE ( replace (replace (phone,'-',''), '(',''), ')', '') as num
from Sales.Customers

select   try_convert (int ,replace (REPLACE ( replace (replace (phone,'-',''), '(',''), ')', ''), ' ', '')),
