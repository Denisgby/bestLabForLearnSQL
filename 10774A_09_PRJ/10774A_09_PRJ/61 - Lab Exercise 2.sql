...
---------------------------------------------------------------------
select so.orderid, sum (sod.unitprice*sod.qty) as salesmount
...
select so.orderid, sum (sod.unitprice*sod.qty) as salesmount, count(*) as noof, AVG (sod.unitprice*sod.qty) as avgprice
from sales.orders  so join sales.orderdetails sod ON so.orderid=sod.orderid
group by so.orderid
order by salesmount desc
