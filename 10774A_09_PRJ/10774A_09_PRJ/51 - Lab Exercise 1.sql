...
---------------------------------------------------------------------
select so.custid, sc.contactname
from Sales.Orders so join Sales.Customers sc ON so.custid=sc.custid
where so.empid=5
group by so.custid, sc.contactname
...
---------------------------------------------------------------------
select so.custid, sc.contactname, sc.city
from Sales.Orders so join Sales.Customers sc ON so.custid=sc.custid
where so.empid=5
group by so.custid, sc.contactname, sc.city
...
select so.custid, YEAR (so.orderdate) as year
